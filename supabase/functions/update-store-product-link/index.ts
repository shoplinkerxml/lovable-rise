...
serve(async (req) => {
  ...
  try {
    ...
    if (!productId || !storeId) {
      return new Response(JSON.stringify({ error: "invalid_body", message: "product_id and store_id required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!existing) {
      const { data: inserted, error: insErr } = await supabase
        .from("store_product_links")
        .insert([{ product_id: productId, store_id: storeId, is_active: allowed.is_active ?? true, custom_price: allowed.custom_price, custom_price_old: allowed.custom_price_old, custom_price_promo: allowed.custom_price_promo, custom_stock_quantity: allowed.custom_stock_quantity, custom_available: allowed.custom_available, custom_name: allowed.custom_name, custom_description: allowed.custom_description, custom_category_id: allowed.custom_category_id }])
        .select("*")
        .maybeSingle();
      if (insErr) {
        return new Response(JSON.stringify({ error: "update_failed", message: insErr.message || "Insert link failed" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      return new Response(JSON.stringify({ link: inserted }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const { data: updated, error: updErr } = await supabase
      ...
      .maybeSingle();
    if (updErr) {
      return new Response(JSON.stringify({ error: "update_failed", message: updErr.message || "Update link failed" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    return new Response(JSON.stringify({ link: updated }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    const msg = (e as unknown as { message?: string })?.message ?? "Update link failed";
