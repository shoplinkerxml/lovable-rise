...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (links.length === 0) return new Response(JSON.stringify({ inserted: 0 }), { status: 200, headers: corsHeaders })
    ...
    const existingSet = new Set((existing || []).map((r: any) => `${String(r.product_id)}__${String(r.store_id)}`))
    const toInsert = links.filter((l) => !existingSet.has(`${l.product_id}__${l.store_id}`))

    if (toInsert.length === 0) return new Response(JSON.stringify({ inserted: 0 }), { status: 200, headers: corsHeaders })
    ...
    if (insErr) return new Response(JSON.stringify({ error: 'insert_failed' }), { status: 500, headers: corsHeaders })

    try {
      const prodIdsUnique = Array.from(new Set(toInsert.map((l) => l.product_id)))
      const storeIdsUnique = Array.from(new Set(toInsert.map((l) => l.store_id)))
      const { data: prodRows } = await supabase
        .from('store_products')
        .select('id,category_id,supplier_id,category_external_id')
        .in('id', prodIdsUnique)

      const supplierIds = Array.from(new Set((prodRows || []).map((r: any) => r?.supplier_id).filter((v) => v != null)))
      const externalIds = Array.from(new Set((prodRows || []).map((r: any) => r?.category_external_id).filter((v) => !!v)))
      ...
      if (supplierIds.length > 0 && externalIds.length > 0) {
        ...
        for (const r of catRows || []) {
          const key = `${String((r as any).supplier_id)}::${String((r as any).external_id)}`
          catByExt[key] = Number((r as any).id)
        }
      }
      ...
      for (const r of (prodRows || [])) {
        ...
        const cid = (r as any)?.category_id != null ? Number((r as any).category_id) : null
        if (cid != null) {
        ...
        const key = sup != null && ext ? `${String(sup)}::${String(ext)}` : null
        catIdByProduct[pid] = key && catByExt[key] ? Number(catByExt[key]) : null
      }
      ...
      if (targetPairs.size > 0) {
        ...
        const existingCatPairs = new Set((existingCats || []).map((r: any) => `${String((r as any).store_id)}__${String((r as any).category_id)}`))
        const missingPairs: Array<{ store_id: string; category_id: number }> = []
        for (const pair of Array.from(targetPairs)) {
          if (!existingCatPairs.has(pair)) {
            ...
            if (sid && Number.isFinite(cidNum)) missingPairs.push({ store_id: sid, category_id: cidNum })
          }
        }

        if (missingPairs.length > 0) {
          const catIds = Array.from(new Set(missingPairs.map((p) => p.category_id)))
          ...
          for (const r of catsInfo || []) {
            extById[Number((r as any).id)] = (r as any)?.external_id != null ? String((r as any).external_id) : null
          }
    ...
    return new Response(JSON.stringify({ inserted: toInsert.length, addedByStore }), { status: 200, headers: corsHeaders })
  } catch (e) {
